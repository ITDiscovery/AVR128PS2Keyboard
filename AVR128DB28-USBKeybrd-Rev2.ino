...
// Keyboard Template Starts with ZX-Spectrum
uint16_t kbdtmp = 1;
// This template has 180 entries
uint16_t tabletop = 173;
  ...
  if( (c>0) & (c<32768)) 
    {
      ...
      Serial.print(c);
      uint16_t e = 0;
      while (lookup[kbdtmp][e][0] != c and e < tabletop) {
        e++;
      }
      Serial.print(" E=");
      Serial.print(e);
      if (e < tabletop) {
        Serial.print(" Conv=");
        Serial.print(lookup[kbdtmp][e][1]); 
        Serial.print(" ");
        ALDLOutput(lookup[kbdtmp][e][1]);  
      }
    }
  }
}
